# -*- coding: utf-8 -*-
# Copyright (c) 2021, Anju Ashin and contributors
...
communication_mapping = {"": "Custom Event", "Event": "Event", "Meeting": "Meeting", "Call": "Phone", "Sent/Received Email": "Email", "Other": "Other"}
...
class CustomEvent(Document):
	...
	def update_communication(self, participant, communication):
		communication.communication_medium = "Custom Event"
...
def get_permission_query_conditions(user):
	...
	return """(`tabCustom Event`.`event_type`='Public' or `tabCustom Event`.`owner`=%(user)s)""" % {
...
def get_events(start, end, user=None, for_reminder=False, filters=None):
	...
	filter_condition = get_filters_cond('Custom Event', filters, [])

	tables = ["`tabCustom Event`"]
	if "`tabEvent Participants`" in filter_condition:
		tables.append("`tabEvent Participants`")

	events = frappe.db.sql("""
		SELECT `tabCustom Event`.name,
				`tabCustom Event`.subject,
				`tabCustom Event`.description,
				`tabCustom Event`.color,
				`tabCustom Event`.starts_on,
				`tabCustom Event`.ends_on,
				`tabCustom Event`.owner,
				`tabCustom Event`.all_day,
				`tabCustom Event`.event_type,
				`tabCustom Event`.repeat_this_event,
				`tabCustom Event`.repeat_on,
				`tabCustom Event`.repeat_till,
				`tabCustom Event`.monday,
				`tabCustom Event`.tuesday,
				`tabCustom Event`.wednesday,
				`tabCustom Event`.thursday,
				`tabCustom Event`.friday,
				`tabCustom Event`.saturday,
				`tabCustom Event`.sunday
		FROM {tables}
		WHERE (
				(
					(date(`tabCustom Event`.starts_on) BETWEEN date(%(start)s) AND date(%(end)s))
					OR (date(`tabCustom Event`.ends_on) BETWEEN date(%(start)s) AND date(%(end)s))
					OR (
						date(`tabCustom Event`.starts_on) <= date(%(start)s)
						AND date(`tabCustom Event`.ends_on) >= date(%(end)s)
					)
				)
				OR (
					date(`tabCustom Event`.starts_on) <= date(%(start)s)
					AND `tabCustom Event`.repeat_this_event=1
					AND coalesce(`tabCustom Event`.repeat_till, '3000-01-01') > date(%(start)s)
				)
			)
		{reminder_condition}
		{filter_condition}
		AND (
				`tabCustom Event`.event_type='Public'
				OR `tabCustom Event`.owner=%(user)s
				OR EXISTS(
					SELECT `tabDocShare`.name
					FROM `tabDocShare`
					WHERE `tabDocShare`.share_doctype='Custom Event'
						AND `tabDocShare`.share_name=`tabCustom Event`.name
						AND `tabDocShare`.user=%(user)s
				)
			)
		AND `tabCustom Event`.status='Open'
		ORDER BY `tabCustom Event`.starts_on""".format(
			tables=", ".join(tables),
			filter_condition=filter_condition,
			reminder_condition="AND coalesce(`tabCustom Event`.send_reminder, 0)=1" if for_reminder else ""
		), {
			"start": start,
			"end": end,
			"user": user,
...
def delete_events(ref_type, ref_name, delete_event=False):
	participations = frappe.get_all("Event Participants", filters={"reference_doctype": ref_type, "reference_docname": ref_name,
		"parenttype": "Custom Event"}, fields=["parent", "name"])

	if participations:
		for participation in participations:
			if delete_event:
				frappe.delete_doc("Custom Event", participation.parent, for_reload=True)
			else:
				total_participants = frappe.get_all("Event Participants", filters={"parenttype": "Custom Event", "parent": participation.parent})

				if len(total_participants) <= 1:
					frappe.db.sql("DELETE FROM `tabCustom Event` WHERE `name` = %(name)s", {'name': participation.parent})
...
def set_status_of_events():
	events = frappe.get_list("Custom Event", filters={"status": "Open"}, fields=["name", "ends_on", "repeat_till"])
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) \
			...
			frappe.db.set_value("Custom Event", event.name, "status", "Closed")
